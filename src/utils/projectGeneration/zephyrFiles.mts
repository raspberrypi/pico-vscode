    ...
    void *user_data)
{
    ...
    if (rsp->body_found)
    {
        size_t cur_len = strnlen(json_response_buffer, sizeof(json_response_buffer));
        if (rsp->body_frag_len > sizeof(json_response_buffer) - cur_len)
        {
            LOG_ERR("JSON Response Buffer out of space: %d needed, %d available", rsp->body_frag_len, sizeof(json_response_buffer) - cur_len);

            return -1;
        }
        else
        {
            strncat(json_response_buffer, rsp->body_frag_start, rsp->body_frag_len);
        }
    }
